...
example usage
    ...
    python3 biasopt_from_bp.py --ntask 1 --tasktype 1vall --nhidden 50 --nepoch 100 --readout tanh
...
source_train = helperfuncs_new.get_dataset(args.dataset, train=True, rotate=True)
source_test = helperfuncs_new.get_dataset(args.dataset, train=False, rotate=True)
...
for n_h in args.nhidden:
    ...
    for task in tasks:
        # initialize weights
        w_in  = helperfuncs_new.get_weight_matrix_in(n_h, ALGO, dataset=args.dataset, task=task)
        w_out = helperfuncs_new.get_weight_matrix_out(n_h, algo=ALGO, dataset=args.dataset, task=task, bias_opt=len(w_idx)==0)
        ws = [w_in, w_out]
    ...
    with open(args.path + "biasopt_newparams_1hl_%s_%s%d_%s_ro=%s.p"%(args.dataset, ALGO, n_h, args.tasktype, args.readout), 'wb') as file:
