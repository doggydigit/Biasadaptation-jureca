...
ENHANCEMENT = 'reweighted'#'enriched'#
...
for ii in range(2):
    ...
    algos_all = ['-'.join(ALGO_12C[ii])]
    algos_1 = [ALGO_12C[ii][0]]

    rwstrs = ['', '_'+ENHANCEMENT]

    for kk, rwstr in enumerate(rwstrs):
        ...
        for jj, (algo_1, algo_12c) in enumerate(zip(algos_1, algos_all)):
            ...
            perfs_1hl = []
            for n_h in NHS:
                namestring_nhx = "deepopt_2hl_%s%s_algo12c=%s_nh12=%d-%d_%s_ro=%s.p"%(DATASET, rwstr, algo_12c, NH, n_h, TASKTYPE, READOUT)
                print(namestring_nhx)
                perfs_nhx.append(load_perf(namestring_nhx))

                namestring_xnh = "deepopt_2hl_%s%s_algo12c=%s_nh12=%d-%d_%s_ro=%s.p"%(DATASET, rwstr, algo_12c, n_h, NH, TASKTYPE, READOUT)
                perfs_xnh.append(load_perf(namestring_xnh))

                namestring_xx = "deepopt_2hl_%s%s_algo12c=%s_nh12=%d-%d_%s_ro=%s.p"%(DATASET, rwstr, algo_12c, n_h, n_h, TASKTYPE, READOUT)
                perfs_xx.append(load_perf(namestring_xx))

                namestring_1hl = "biasopt_1hl_%s%s_%s%d_%s_ro=%s.p"%(DATASET, rwstr, algo_1, n_h, TASKTYPE, READOUT)
        ...
        colour = colours[jj] if kk == 0 else 'DarkGrey'
        ...
        for jj, algo_12c in enumerate(algos_all):
            ax.plot(np.arange(len(NHS))+0.5, perfs_xnh_c[algo_12c],
                ls='--', marker="s", c=colour, lw=lwidth, ms=markersize, label=algo_12c)
...
figname = os.path.join(paths.fig_path, 'biasopt/res_deepopt_reweighted_%s_%s_ro=%s'%(DATASET, TASKTYPE, READOUT))
